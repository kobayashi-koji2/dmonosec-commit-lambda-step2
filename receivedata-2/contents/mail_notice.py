...
def mailNotice(hist_list, device_info, user_table, account_table, notification_hist_table):
    ...
    for notification_settings in notification_settings_list:
        ...
        for i, hist_list_data in enumerate(hist_list):
            ...
            if hist_list_data.get('hist_data', {}).get('event_type') == "di_change" and notification_settings.get('event_trigger') == "di_change" and\
                notification_settings.get('terminal_no') == hist_list_data.get('hist_data', {}).get('terminal_no'):
                di_state = diNameToState(hist_list_data.get('hist_data', {}).get('terminal_state_name'), device_info)
                if notification_settings.get('change_detail') == di_state:
                    terminal_name = hist_list_data.get('hist_data', {}).get('terminal_name')
                    terminal_state_name = hist_list_data.get('hist_data', {}).get('terminal_state_name')
                    event_detail = f"""
                        　【接点入力変化】
            ...
            # 接点出力
            if hist_list_data.get('hist_data', {}).get('event_type') == "do_change" and notification_settings.get('event_trigger') == "do_change" and\
                notification_settings.get('terminal_no') == hist_list_data.get('hist_data', {}).get('terminal_no'):
                terminal_name = hist_list_data.get('hist_data', {}).get('terminal_name')
                terminal_state_name = hist_list_data.get('hist_data', {}).get('terminal_state_name')
                event_detail = f"""
            ...
            # デバイス状態（バッテリーニアエンド）
            elif hist_list_data.get('hist_data', {}).get('event_type') == "battery_near" and notification_settings.get('event_trigger') == "device_change" and\
                notification_settings.get('event_type') == "battery_near":
            ...
            # デバイス状態（機器異常）
            elif hist_list_data.get('hist_data', {}).get('event_type') == "device_abnormality" and notification_settings.get('event_trigger') == "device_change" and\
                notification_settings.get('event_type') == "device_abnormality":
                if hist_list_data.get('hist_data', {}).get('occurrence_flag') == 1:
                    event_detail = f"""
                        　【機器異常（発生）】
                        　機器異常が発生しました。
            ...
            # デバイス状態（パラメータ異常）
            elif hist_list_data.get('hist_data', {}).get('event_type') == "parameter_abnormality" and notification_settings.get('event_trigger') == "device_change" and\
                notification_settings.get('event_type') == "parameter_abnormality":
                if hist_list_data.get('hist_data', {}).get('occurrence_flag') == 1:
                    event_detail = f"""
                        　【パラメータ異常（発生）】
                        　パラメータ異常が発生しました。
            ...
            # デバイス状態（FW更新異常）
            elif hist_list_data.get('hist_data', {}).get('event_type') == "fw_update_abnormality" and notification_settings.get('event_trigger') == "device_change" and\
                notification_settings.get('event_type') == "fw_update_abnormality":
                if hist_list_data.get('hist_data', {}).get('occurrence_flag') == 1:
                    event_detail = f"""
                        　【FW更新異常（発生）】
                        　FW更新異常が発生しました。
            ...
            # 電源ON
            elif hist_list_data.get('hist_data', {}).get('event_type') == "power_on" and notification_settings.get('event_trigger') == "device_change" and\
                notification_settings.get('event_type') == "power_on":
                event_detail = f"""
                    　【電源ON】
                    　デバイスの電源がONになりました。
                """
                ...
                if "link_terminal_no" in hist_list_data.get('hist_data', {}):
                    if notification_settings.get('terminal_no') == hist_list_data.get("hist_data", {}).get("terminal_no"):
                        if hist_list_data.get('hist_data', {}).get('event_type') == "manual_control":
                            ...
                            link_terminal_state_name = hist_list_data.get('hist_data', {}).get('link_terminal_state_name')
                            control_exec_user_name = hist_list_data.get('hist_data', {}).get('control_exec_user_name')
                            event_detail = f"""
                                　【画面操作による制御（成功）】
                            ...
                            automation_trigger_device_name = hist_list_data.get('hist_data', {}).get('automation_trigger_device_name', automation_trigger_imei)
                            automation_trigger_event_type = hist_list_data.get('hist_data', {}).get('automation_trigger_event_type')
                            automation_trigger_event_detail_state = hist_list_data.get('hist_data', {}).get('automation_trigger_event_detail_state')
                            automation_trigger_event_detail_flag = hist_list_data.get('hist_data', {}).get('automation_trigger_event_detail_flag')
                            event_label = automationSetting(automation_trigger_event_type, automation_trigger_event_detail_state, automation_trigger_event_detail_flag)
                            event_detail = f"""
                                　【連動設定による制御（成功）】
                                　制御信号（{terminal_name}）がデバイスに届き、{link_terminal_name}が{link_terminal_state_name}に変化しました。
                    ...
                    if hist_list_data.get('hist_data', {}).get('event_type')  == "manual_control":
                        if hist_list_data.get("hist_data", {}).get("terminal_no") == notification_settings.get("terminal_no"):
                            terminal_name = hist_list_data.get('hist_data', {}).get('terminal_name')
                            control_exec_user_name = hist_list_data.get('hist_data', {}).get('control_exec_user_name')
                            event_detail = f"""
                                　【画面操作による制御（成功）】

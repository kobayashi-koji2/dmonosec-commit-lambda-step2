...
def createHistListData(recv_data, device_info, event_info, device_relation_table, group_table):
    logger.debug(
        f"createHistListData開始 recv_data={recv_data}, device_info={device_info}, event_info={event_info}"
    )
    # グループ情報取得
    group_list = []
    group_list = ddb.get_device_group_list(
        device_info["device_id"], device_relation_table, group_table
    )
...
def updateCurrentStateInfo(current_state_info, event_info, event_datetime):
    logger.debug(
        f"updateCurrentStateInfo開始 current_state_info={current_state_info}, event_info={event_info}, event_datetime={event_datetime}"
    )
    di_state = [
        "di1_state",
        "di2_state",
...
def eventJudge(
    ...
    if recv_data["message_type"] in ["0001", "0011", "0012"]:
        ...
        if (init_state_flg) or (
            not init_state_flg and hist_signal_state != device_current_state["signal_state"]
        ):
            current_state_info = updateCurrentStateInfo(
                current_state_info, event_info, event_datetime
            )

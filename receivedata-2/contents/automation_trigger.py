...
def automationTrigger(hist_list, device_info):
    ...
    for hist in hist_list:
        ...
        if hist["hist_data"]["event_type"] == "di_change":
            event_detail = diNameToEventDetail(
                hist["hist_data"]["terminal_state_name"], device_info
            )
            automation.automation_control(
                hist["device_id"],
                "di_change_state",
                hist["hist_data"]["terminal_no"],
                event_detail,
                None,
            )
        # バッテリー残量、デバイス異常、パラメータ異常、FW更新異常
        elif hist["hist_data"]["event_type"] in [
            "battery_near",
            "device_abnormality",
            ...
            automation.automation_control(
                hist["device_id"],
                hist["hist_data"]["event_type"],
                None,
                None,
                hist["hist_data"]["occurrence_flag"],
            )
        # 電源オン
        elif hist["hist_data"]["event_type"] == "power_on":
            automation.automation_control(
                hist["device_id"], hist["hist_data"]["event_type"], None, None, 0
            )
        # その他
        else:
            continue

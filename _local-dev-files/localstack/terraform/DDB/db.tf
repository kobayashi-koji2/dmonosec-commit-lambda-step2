...
#sigfox-id管理テーブル
resource "aws_dynamodb_table" "sigfox_id" {
  name           = "${var.global_name}-ddb-m-office-sigfox-id"
  hash_key       = "sigfox_id"
  stream_enabled = "false"
  table_class    = "STANDARD"
...
resource "aws_dynamodb_table" "device_announcements" {
  ...
  global_secondary_index {
    ...
    name            = "identification_id_announcement_type_index"
    projection_type = "ALL"
  }

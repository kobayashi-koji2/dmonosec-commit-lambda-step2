    ...
    if 'gsi_name' in kwargs:
        ...
        if 'sk_prefix' in kwargs:
            response = table.query(
                ...
                KeyConditionExpression=Key('key2').eq(pk) & Key('key1').begins_with(kwargs['sk_prefix'])
            ).get("Items", {})
        #GSI PK検索
        else:
            response = table.query(
        ...
        if 'sk_prefix' in kwargs:
            response = table.query(
                KeyConditionExpression=Key('key1').eq(pk) & Key('key2').begins_with(kwargs['sk_prefix'])
            ).get("Items", {})
...
#モノセコムユーザ情報取得
def get_user_info(account_id,contract_id,user_table):
    user_table = user_table.query(
        IndexName='account_id_index',
        KeyConditionExpression=Key('account_id').eq(account_id) & Key('contract_id').eq(contract_id)
    )
    return user_table
...
def get_account_info(user_id,account_table):
    account_table = account_table.query(
        ...
        KeyConditionExpression=Key('auth_id').eq(user_id)
    )
